-- =============================================================================
-- WITHDRAWAL AVAILABILITY HOLDS
-- =============================================================================

create table if not exists public.withdrawal_holds (
  id            uuid primary key default gen_random_uuid(),
  settlement_id uuid references public.league_race_settlements(id) on delete cascade,
...
create index if not exists idx_withdrawal_holds_user_id
  on public.withdrawal_holds (user_id);

create index if not exists idx_withdrawal_holds_available_at
  on public.withdrawal_holds (available_at);

alter table public.withdrawal_holds enable row level security;

drop policy if exists "wh_select_own" on public.withdrawal_holds;
create policy "wh_select_own" on public.withdrawal_holds
  for select to authenticated
  using (auth.uid() = user_id);

drop policy if exists "wh_select_admin" on public.withdrawal_holds;
create policy "wh_select_admin" on public.withdrawal_holds
  for select to authenticated
  using (
    exists (
...
begin
  ...
  loop
    if coalesce((v_item ->> 'held')::boolean, false) then
      ...
      insert into public.withdrawal_holds (
        settlement_id,
        league_id,
        race_id,
        user_id,
      ...
      values (
        ...
        (v_item ->> 'userId')::uuid,
        (v_item ->> 'amount')::numeric,
        'Settlement payout withdrawal review hold',
  ...
  set
    ...
    notes = concat_ws(
      ' ',
      p_notes,
      'Normal payouts were credited immediately; withdrawal availability is delayed by 24 hours.'
    ),
    settled_at = now()
  where id = v_settlement_id;
